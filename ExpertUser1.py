...
class ExpertUser1:
  ...
  def add_userdata(self,freq, tag_fr, tag_fu, tag_eu, tag_t, tag_d):
        self.tas_sum += freq
	self.tag_sum += 1.0
  	if not(tag_fu == None): self.tag_fu_sum.append(  tag_fu*freq )
	else: self.tag_fu_trust += 1
	if not(tag_fr == None): self.tag_fr_sum.append(  tag_fr*freq )
	else: self.tag_fr_trust += 1
	if not(tag_d == None): self.tag_d_sum.append(  tag_d*freq )
	else: self.tag_d_trust += 1
	if not(tag_eu == None): self.tag_eu_sum.append(  tag_eu*freq )
	else: self.tag_eu_trust += 1
	if not(tag_t == None): self.tag_t_sum.append(  tag_t*freq )
